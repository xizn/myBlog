...
function contrastMutedOn(hex, primaryText) {
  ...
  for (const color of ['#eae6de', '#e2ded6', '#d8d4cc', '#f0ece4', '#f5f4f0']) {
    if (contrastRatio(color, hex) >= 4.5) return color;
  }
  return '#f5f4f0';
}
...
console.log('\n=== 无背景图（默认浅色主题）===');
allOk = check('正文 on 页面背景', '#1c1b19', bgColor) && allOk;
allOk = check('次要文字 on 页面背景', '#5a554e', bgColor) && allOk;
allOk = check('输入框文字 on 白底', '#1c1b19', '#ffffff') && allOk;

console.log('\n=== 有背景图 · 暗色图（imageDark，磨砂玻璃）===');
const imageDarkCard = mixHexWithBlack(bgColor, 0.58);
const imageDarkElevated = mixHexWithBlack(bgColor, 0.48);
...
allOk = check('输入框文字 on 白底', '#1c1b19', '#ffffff') && allOk;
console.log('OK 卡片 alpha: 0.56（背景可透出）');

console.log('\n=== 有背景图 · 亮色图（imageLight，磨砂玻璃）===');
const mintBg = mixHexWithWhite('#f4f8f5', 0.2);
const coolBg = mixHexWithWhite('#f0f4f8', 0.2);
allOk = check('薄荷正文 on 卡片', contrastTextOn(mintBg), mintBg) && allOk;
allOk = check('薄荷次要 on 卡片', '#5a554e', mintBg) && allOk;
allOk = check('清冷正文 on 卡片', contrastTextOn(coolBg), coolBg) && allOk;

const mintCard = mintBg;
const coolCard = coolBg;
const tintDiff = mintCard !== coolCard;
console.log(tintDiff ? 'OK 不同主题色产生不同卡片色' : 'FAIL 主题色未反映到卡片');
allOk = tintDiff && allOk;
